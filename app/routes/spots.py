...
def get_spots():
    ...
    if locality_name:
        ...
        area_one_spot_query = Spot.area_one.has(short_name=area_one_name)
...
def search_spots():
    ...
    limit = request.args.get("limit") if request.args.get("limit") else 50
...
def add_spot_wdscript():
    ...
    if alternative:
        full_description += "\n\n%(title)s is also known as %(alternative)s." % {
            "title": name,
...
def add_spot():
    ...
    locality, area_2, area_1, country, latitude, longitude = (
        None,
        None,
        None,
        None,
...
def delete_spot():
    ...
    beach = Spot.query.filter_by(id=id).options(joinedload(Spot.images)).first_or_404()
...
def get_recs():
    ...
    spots = (
        Spot.query.filter(Spot.id.not_in(spots_been_to))
...
def nearby_locations():
    ...
    for result in results:
        ...
        if result.locality and result.locality.url:
            temp_data["locality"] = {"url": result.locality.url}
        else:
