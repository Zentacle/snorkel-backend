...
def get_area_two():
    ...
    sq = (
        ...
        .group_by(table.area_one_id, table.country_id, table.area_two_id)
        .subquery()
    )
    localities = (
        db.session.query(
        ...
        .join(
            sq,
            and_(
                sq.c.area_one_id == AreaTwo.area_one_id,
                sq.c.country_id == AreaTwo.country_id,
                sq.c.area_two_id == AreaTwo.id,
            ),
        )
        .order_by(db.desc("count"))
    )
...
def get_area_one():
    ...
    localities = (
        ...
        .join(
            sq,
            and_(sq.c.area_one_id == AreaOne.id, sq.c.country_id == AreaOne.country_id),
        )
        .order_by(db.desc("count"))
    )
    if country_short_name:
...
def get_wildcard_locality(country, area_one):
    locality = AreaOne.query.filter(
        and_(AreaOne.short_name == area_one, AreaOne.country.has(short_name=country))
    ).first_or_404()
    data = []
