...
def user_signup():
    ...
    password = bcrypt.hashpw(unencrypted_password.encode(
...
def add_spot():
    ...
    spot = Spot.query.filter(
        and_(Spot.name == name, Spot.location_city == location_city)).first()
    if spot:
...
def add_review():
    ...
    visibility = request.json.get('visibility') if request.json.get(
    ...
    if not spot.num_reviews:
        ...
        new_rating = str(round(
            ((float(spot.rating) * (spot.num_reviews*1.0)) + rating) / (spot.num_reviews + 1), 2))
...
def get_reviews():
    ...
    reviews = Review.query.options(joinedload('user')).order_by(
        Review.date_posted.desc()).filter_by(beach_id=beach_id).all()
    output = []
...
def get_summary_reviews_helper(beach_id):
    reviews = db.session.query(db.func.count(Review.rating), Review.rating).filter_by(
        beach_id=beach_id).group_by(Review.rating).all()
